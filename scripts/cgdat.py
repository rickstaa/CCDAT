...
from qt_ui import Ui_output_settings
...
class DataAnalyserGUI(Ui_MainWindow):
    ...
    #################################################
    ### Toggle time range settings fields         ###
    #################################################
    def toggle_time_range_settings(self):
        '''Qt slot function used to disable and enable the time range settings fields based on whether the time range filter option is enabled.
        '''

        ### Enable input field if time file filter is enabled ###
        if self.time_range_toggle.checkState() :  # If checked
            self.time_range_value.setEnabled(1)
        else:
            self.time_range_value.setEnabled(0)

    #################################################
    ### Toggle player filter settings fields      ###
    #################################################
    def toggle_player_filter_settings(self):
        '''Qt slot function used to disable and enable the time range settings fields based on whether the time range filter option is enabled.
        '''

        ### Enable input field if time file filter is enabled ###
        if self.player_filter_toggle.checkState() :  # If checked
            self.player_filter_choicer.setEnabled(1)
        else:
            self.player_filter_choicer.setEnabled(0)
    ...
    def start_data_analysis(self):
        ...
        worker = Worker(self.analyse_data)  # Any other args, kwargs are passed to the run function

        ### Connect status signals ###
        # worker.signals.result.connect(self.print_output)  # Results signal slot
            ...
            #################################################
                ...
                for condition_text in self.condition_line_edit:
                    ...
                    if self.input_file_path.text():
                        ...
                        begin_times = pd.to_timedelta(df_time['Start Time'])  # Get start times out of dataframe (timedelta format)
                        end_times = pd.to_timedelta(df_time['End Time'])  # Get end times out of dataframe (timedelta format)
                        start_time_offset = pd.to_timedelta(df_time[(df_time["Name"] == "Start")]["Start Time"].tolist()[0]) # Get time offset
