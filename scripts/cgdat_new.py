...
# TODO: No clean for second data file import
...
# TODO: Update dialog in different thread
# TODO: Empty condition error
# TODO: Pandas not saving time right from 0 to 100
...
class DataAnalyserGUI(Ui_MainWindow):
    ...
    def get_input_file(self):
        ...
        if not (fileName == "."):  # If not empty
            ...
            ### Create input file progress dialog ###
            ...
            if not result:
                ...
                self.time_file_toggle.setEnabled(0) # Disable file path toggle
            ...
            self.player_filter_toggle.setEnabled(0) # Disable player toggle
            self.player_filter_drop_down_menu.setEnabled(0)
            self.player_filter_drop_down_menu.setText("Please import a valid data file to see the available players")
    ...
    def analyse_input_data_file(self, *args, **kwargs):
        ...
        if not ("Name" in self.df.columns):
            ...
            ### Return that the result that the data file is correct ###
            return (True,)
    ...
    def data_input_worker_finished(self, result):
        ...
        if result[0]:  # If import was successful
            ...
            self.time_file_toggle.setEnabled(1) # Enable file path toggle
    ...
    def start_data_analysis(self):
        ...
        if (self.player_filter_toggle.checkState() == 2):
        ...
        if test_result:
            ...
            if not (self.player_filter_toggle.checkState() == 2):  # Player filter not selected
            ...
            elif (self.player_filter_toggle.checkState() == 2) and (len(filtered_players) == 0):  # If player filter was enabled but no player selected
                ...
                ### Start workers ###
                for ii in range(0,len(filtered_players)):  # Loop through players
    ...
    def analyse_data(self, *args, **kwargs):
        ...
        for condition_text in self.condition_line_edit:
            ...
            if self.time_file_toggle.isChecked() and not (self.input_file_path.text() == ''):
                ...
                df_sections = [df_tmp.between_time(i, j) for (i,j) in zip(begin_times, end_times)]
                df_tmp = pd.concat(df_sections) # Add all the dataframes of the time_sections together again

            #################################################
            ### Filter on player name is player name exists##
            #################################################
            if not (player_name == None):
                df_tmp = df_tmp[df_tmp["Name"]==player_name]  # Only keep data belonging to the player specified in player_name

            #################################################
            ### Check data against conditions################
            #################################################
            try:
                df_condition = df_tmp[eval(condition)]
...
# TODO: Remove message box from analysis
